...
class TCP:
    def set_options(self, options):
        self.tcp_port = options.tcp_port

    def make_pcap(self, pcap, req, resp):
        stream = pcap.tcp_conn_to_server(self.tcp_port)
    ...
    def make_CTS_pcap(self, pcapname, fileobj, content_encoding):
        pcap = PCAP(pcapname)
        ...
        req = data
        resp = randtext(min=256, max=512)

        self.make_pcap(pcap, req, resp)

        pcap.close()

    def make_STC_pcap(self, pcapname, fileobj, content_encoding):
        pcap = PCAP(pcapname)

        fp = getattr(fileobj, 'get_{0}'.format(content_encoding))
        data = fp()

        req = randtext(min=256, max=512)
        resp = data
    ...
    def run(self, fileobj):
        for method in [ 'CTS', 'STC' ]:
            for encoding in [ 'raw' ]:
                pcapname = 'output/{0}/TCP_{1}_{2}.pcap'.format(fileobj.filename, method, encoding)
                print "[*] {0} ...".format(pcapname)
                fp = getattr(self, 'make_{0}_pcap'.format(method))
                fp(pcapname, fileobj, encoding)
...
def parse_args(args):
    ...
    parser.add_option("--tcp-port", type="int", default=9999, help="Port number for TCP connections [default: %default]", metavar="PORT")
